					...
					success(function(data, status) {
						...
						angular.forEach($scope.treeJSONarray, function(value, key) {
							...
							if(key == 1) {
								...
								menuTreeHTML += '<li><a href="/developer-docs/#/' + $scope.treeJSONarray[key].path + '/' + $scope.treeJSONarray[key].name + '" title="Home">' + linkName + '</a></li>';
							}
//							else if(key == 2) {
//								lastFolder = $scope.treeJSONarray[key].path.split("/");
//								menuTreeHTML += '<li>' + lastFolder[lastFolder.length - 1].toUpperCase() + '<ul>';
...
//							}
								...
								if($scope.treeJSONarray[key].depth > lastDepth) {
									...
									menuTreeHTML += '<li><a href="/developer-docs/#/' + $scope.treeJSONarray[key].path + '/' + $scope.treeJSONarray[key].name + '" title="' + $scope.treeJSONarray[key].name + '">' + linkName + ' TTT</a></li>';											
								}

								if($scope.treeJSONarray[key].depth == lastDepth) {
									lastFolder = $scope.treeJSONarray[key].path.split("/");
									linkName = $scope.treeJSONarray[key].name.replace(/\.md$/, "").charAt(0).toUpperCase() + $scope.treeJSONarray[key].name.replace(/\.md$/, "").slice(1).toLowerCase();
									menuTreeHTML += '<li><a href="/developer-docs/#/' + $scope.treeJSONarray[key].path + '/' + $scope.treeJSONarray[key].name + '" title="' + $scope.treeJSONarray[key].name + '">' + linkName + ' RRR</a></li>';											
